...
num_files=$(rsync -rtvhL --min-size=1M --timeout=60 --no-perms --stats "$STORAGE_MOUNT"/TeslaCam/SentryClips "$STORAGE_MOUNT"/TeslaCam/SavedClips "$ARCHIVE_MOUNT"/ | awk '/files transferred/{print $NF}')
